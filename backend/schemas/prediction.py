...
class ShortfallPredictionRequest(BaseModel):
    ...
    equipment_availability: float = Field(..., ge=0, le=100, description="Equipment availability percentage")
    equipment_downtime: float = Field(..., ge=0, description="Downtime hours logged across equipment")
    equipment_utilization: Optional[float] = Field(80.0, ge=0, le=100, description="Equipment utilization percentage")
    maintenance_hours: float = Field(..., ge=0, description="Maintenance bay hours")
    haulage_truck_count: int = Field(..., ge=0, description="Active haulage truck units")
    ...
    production_target: float = Field(..., gt=0, description="Daily production mandate in tonnes")
