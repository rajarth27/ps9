...
export function ExplorationMap({
  ...
  useEffect(() => {
    ...
    const initialCenter = selectedZone
    ...
    const map = L.map(mapRef.current, {
      ...
      // Prevent Leaflet zoom-transition lifecycle crash
      fadeAnimation: false,
      zoomAnimation: false,
    ...
    // One layer group for all dynamic markers
    const layers = L.layerGroup().addTo(map);
    layersRef.current = layers;

    // =========================================================
    // MOIL OPERATING MINES
    // =========================================================
  ...
  // =========================================================
  // UPDATE EXPLORATION ZONE MARKERS
  // =========================================================
  useEffect(() => {
    ...
    layers.eachLayer((layer) => {
      if (layer.options?.isExplorationZone) {
    ...
    zones.forEach((zone) => {
      ...
      const probColor =
        zone.reserveProbability >= 85
          ? '#10b981'
          : zone.reserveProbability >= 70
            ? '#38bdf8'
            : '#f59e0b';
      ...
      marker.bindPopup(`
        ...
        ">
          ...
          Mine Sector:
          <strong>${zone.mineName}</strong><br/>
          ...
          Estimated Reserve:
          <strong>${zone.estimatedReserveMT} MT</strong><br/>

          Predicted Grade:
          <strong>${zone.predictedGrade}% Mn</strong><br/>
  ...
  // =========================================================
  // MOVE MAP WHEN A ZONE IS SELECTED
  // =========================================================
  useEffect(() => {
    const map = mapInstanceRef.current;
    ...
    // Stop any existing animation before moving
    map.stop();
  ...
  return (
    <div
      className={`relative h-[480px] w-full rounded-xl overflow-hidden border border-slate-800 ${className}`}
    >
      <div
        ref={mapRef}
      ...
      <div className="absolute top-3 right-3 z-[1000] glass-panel p-3 rounded-lg border border-slate-700/80 text-xs space-y-2 bg-mining-900/90 shadow-xl max-w-xs pointer-events-auto">
        ...
        <div className="space-y-1 text-[11px] text-slate-300">
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-slate-800 text-[10px] text-slate-400">
            ...
            <span>
              Operating Mine Production Shafts
            </span>
          </div>
